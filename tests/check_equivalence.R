...
#'
#' It assumes that the results were previously generated using the scripts in :
#' - run_examples_original_script.R
#' - run_examples_package.R
#' *****
#'
#' ## Load data
...
for (kk in seq_along(dirs)){
  ...
  data2 <- readRDS(paste0("./zz_original_scripts/results/", dirs[kk], "/final_KOMODO2_list.rds")) # created using original scripts
  ...
  okvec <- c(okvec, all(names(data1) == names(data2)))
  ...
  okvec <- c(okvec, all(sapply(data2, class) == sapply(data1, class)))
  ...
  #'
  ## ---------------------------------------------------------------------------------------------------------------
  tolower(data1$ontology) == tolower(data2$ontology)
  okvec <- c(okvec, tolower(data1$ontology) == tolower(data2$ontology))
  ...
  okvec <- c(okvec, identical(data1$x, data2$x))
  ...
  cbind(a=data1$greaterthanzero[1:5],
  ...
  okvec <- c(okvec, identical(tmp, data1$greaterthanzero))
  ...
  okvec <- c(okvec, all(mapply(function(a,b){a %in% b},
                               a = data1$mode[idx],
                               b = tmp)))
