...
```{r Heatmaps, echo = TRUE, warnings = FALSE, message = FALSE}
...
clade_order <- stats::order.dendrogram(tree2)

mat     <- as.matrix(norm[, 1:ncol(norm)])
ord.mat <- mat[tree$tip.label, ]

#ord.mat

my_palette <- RColorBrewer::colorRampPalette(c("white", "blue"))(n = 51)


jColors <- data.frame(LABEL = levels(mynames$group),
                      COLOR = I(RColorBrewer::brewer.pal(nlevels(mynames$group), 
                                                         name = 'Set1')))
...
distance2 <- stats::dist(t(ord.mat), method = "euclidean")
#cluster2 = hclust(distance2, method = "ward.D2")

cluster2_final <- dendextend::set(
  stats::as.dendrogram(
    stats::hclust(distance2, method = "average")), 
  what  = "branches_lwd", 
  value = 1)
...
heatmaply::heatmaply(
  ...
  file          = "heatmap.html",
  Rowv          = tree_final,
  Colv          = stats::hclust(distance2, method = "average"),
  col           = my_palette,
  row_dend_left = TRUE,
...
for (i in seq_along(df_cutoff$name)){
  #for (i in seq_along(df_cutoff$name[1:length(df_cutoff[, 1])]) { # filter to plot a lot
