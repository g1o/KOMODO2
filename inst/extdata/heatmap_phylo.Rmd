...
jColors <- data.frame(LABEL = levels(mynames$group),
                      COLOR = KOMODO2_brewer_pal(nlevels(mynames$group), 
                                                 name = 'Set1'),
...
tmp <- data.frame(species = rownames(ord.mat), stringsAsFactors = FALSE) %>%
  dplyr::mutate(group = mynames$group[match(.data$species, mynames$shortName)],
                COLOR = jColors$COLOR[match(.data$group, jColors$LABEL)])
