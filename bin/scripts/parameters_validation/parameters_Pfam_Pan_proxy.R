...
KOMODO2 <- list(annotation_files_dir = "../../data/Pfam/", #Directory where annotation files are located. If not provided, file described in variable "dataset.info" should contain absolute paths to annotation files
                output.dir = "../../results/Pfam_Pan_proxy_2/", #output directory for results

                dataset.info = "../../data/metadata/Pfam_metadata_Pan_proxy.txt", #genome metadata file, it should contain at least, for each genome: 1) path for annotation data; 2) phenotype data (numeric); 3) normalization data (numeric)

                x.column = 2, #which column contains phenotype data?

                ontology = "other", #which dictionary data type?

                dict.path = "../../data/dics/Pfam.dic", #file for dictionary file (two-column file containing annotation IDs and their descriptions, not needed for GO
