...
KOMODO2 <- list(annotation_files_dir = "/Users/chico/projects/KOMODO2/development/KOMODO2/bin/validation/data/gene2GO/", #Directory where annotation files are located. If not provided, file described in variable "dataset.info" should contain absolute paths to annotation files
                output.dir = "../validation/results/GO_less_H_sapiens/", #output directory for results

                dataset.info = "../validation/data/metadata/GO_metadata_less_H_sapiens.txt", #genome metadata file, it should contain at least, for each genome: 1) path for annotation data; 2) phenotype data (numeric); 3) normalization data (numeric)
                ...
                tree_path = "../validation/data/trees/tree_genome_IDs_less_H_sapiens.nwk", #path for tree file
