...
KOMODO2 <- list(annotation_files_dir = "../../data/Pfam/", #Directory where annotation files are located. If not provided, file described in variable "dataset.info" should contain absolute paths to annotation files
                ...
                dataset.info = "../../data/metadata/Pfam_metadata_less_H_sapiens.txt", #genome metadata file, it should contain at least, for each genome: 1) path for annotation data; 2) phenotype data (numeric); 3) normalization data (numeric)

                x.column = 2, #which column contains phenotype data?

                ontology = "other", #which dictionary data type?

                dict.path = "../../data/dics/Pfam_less_H_sapiens.dic", #file for dictionary file (two-column file containing annotation IDs and their descriptions, not needed for GO
                ...
                tree_path = "../../data/trees/tree_genome_IDs_less_H_sapiens.nwk", #path for tree file
